...
const TrackBooking = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bookings/mybookings`, {
  ...
  const handleCompleteJourney = async (bookingId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bookings/${bookingId}/complete`, {
        method: 'POST',
        headers: {
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (reason && reason.length >= 5) {
                            fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bookings/${booking._id}/delay`, {
                              method: 'POST',
                              headers: {
                                'Authorization': `Bearer ${token}`,
                                'Content-Type': 'application/json',
