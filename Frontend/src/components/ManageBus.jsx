...
const ManageBus = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/buses`, {
      headers: {
  ...
  const handleUpdate = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/buses/${editingBus._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (busId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/buses/${busId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
