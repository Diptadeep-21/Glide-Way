...
const ManageSchedule = () => {
  ...
  const fetchBuses = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bus/mybuses`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bus/add`, formDataToSend, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (busId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/bus/${busId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleImageError = (e) => {
    ...
    e.target.src = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/uploads/no-image.png`;
  };
              ...
              <tr key={bus._id} className="text-center">
                ...
                <td className="px-4 py-2 border">
                  {bus.image ? (
                    ...
                    <img
                      src={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/uploads/no-image.png`}
                      alt="No Image"
                      className="w-[50px] h-[50px] object-cover mx-auto rounded"
