...
const ManageRoute = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/routes`, {
      headers: {
  ...
  const handleDelete = async (route) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/routes`, {
        method: 'DELETE',
        headers: {
