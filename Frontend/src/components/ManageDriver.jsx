...
const ManageDriver = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/drivers`, {
      headers: {
  ...
  const handleUpdate = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/drivers/${editingDriver._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (driverId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/admin/drivers/${driverId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
